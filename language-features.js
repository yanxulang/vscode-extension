const KEYWORDS = [
  ...
  ["承", "指定父类"], ["父", "绑定当前定义类的直接父类方法"], ["协", "声明结构协议"], ["纳", "声明类纳入协议"],
  ["域", "声明类字段"], ["公", "声明公开成员"], ["私", "声明私有成员"],
  ["只", "声明只读字段"], ["静", "声明静态成员"], ["引", "导入文件或标准模块"],
  ["试", "开始错误捕获块"], ["救", "处理结构化错误"], ["抛", "抛出错误"],
  ["是", "判断值是否符合类型并支持分支收窄"], ["且", "逻辑与运算"], ["或", "逻辑或运算"], ["非", "逻辑非运算"]
...
const TYPES = [
  ...
  ["对象", "对象类型"], ["模块", "模块类型"], ["误", "结构化错误类型"], ["字节串", "不可变任意二进制数据类型"], ["套接字", "TCP/UDP 宿主资源类型"], ["任意", "任意值类型"]
...
const STANDARD_MODULES = [
  ...
  "随机", "标识", "模板", "校验", "Base64", "正则", "URL", "日期", "套接字", "字节"
].map((label) => ({
  label,
  kind: "module",
  detail: `标准:${label}`,

...
当前扩展版本为`1.2.0`，支持言序核心`1.1.4`及兼容的 1.x 版本。扩展会索引工作区、递归导入的外部模块和当前 CLI 的标准库，并为全部用户声明提供跨文卷补全、导航与拼音检索；核心语言和 CLI 无需改动。
...
- 递归索引`引「相对路径」`和`引「包:名称」`引用的工作区内外模块；包导入只读解析已有清单、锁文件与本地缓存，仅向使用方暴露模块的`公`有声明；

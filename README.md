...
当前扩展版本为`1.1.3`，支持言序核心`1.1.3`及兼容的 1.x 版本。扩展已同步`父.方法`、`值 是 类型`、结构化网络/套接字错误、带预算的 HTTP/HTTPS 请求及 TCP/UDP 片段。1.1.3 的片段说明区分网络外连、TCP 监听和 UDP 绑定权限，并包含全部 22 个稳定标准模块的补全。
